...
function AddToCart() {
  ...
  const handleBuyNow = () => {
    ...
    navigate('./components/BuyNow');  // Replace '/buy-now' with the actual path of your Buy Now page
  };

  useEffect(() => {
    // Assuming 'Cart' is the array of products in your context or state
    const initialProductId = 1;
    const initialProduct = Cart.find((product) => product.id === initialProductId);
